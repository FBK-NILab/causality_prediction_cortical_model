...

{
	...
	double e2eJ322; //synaptic efficacy from area 3 to area 2
	double e2iJ322;

	double e2eJ123; //synaptic efficacy from area 1 to area 3
	double e2iJ123;
	...
	mwSize  *A322;  //same from 3 to 2
	mwSize  *Aei322;  //same from 3 to 2
	mwSize  *A123;  //same from 1 to 3
	mwSize  *Aei123;  //same from 1 to 3
	...
	mwSize  *Ncon322;  // same excitatory-excitatory from 3 to 2
	mwSize  *Neicon322; // same excitatory-inhibitory from 3 to 2
	mwSize  *Ncon123;  // same excitatory-excitatory from 1 to 3
	mwSize  *Neicon123; // same excitatory-inhibitory from 1 to 3
	...
	// excitatory inter areal 3
	double *iX3;
	double  iX30;
	double  iX3_k1;
	double  iX3_k2;

	//exitatory to inhibitory inter areal 3
	double *uX3;
	double  uX30;
	double  uX3_k1;
	double  uX3_k2;
	...
	//interareal excitatory currents 3
	double *iI3;
	double  iI30;
	double  iI3_k1;
	double  iI3_k2;

	// inter areal excitatory to inhibitory 3
	double *uI3;
	double  uI30;
	double  uI3_k1;
	double  uI3_k2;
	...
	e2eJ322 = *mxGetPr(prhs[60]);
	e2iJ322 = *mxGetPr(prhs[61]);

	e2eJ123 = *mxGetPr(prhs[62]);
	e2iJ123 = *mxGetPr(prhs[63]);

	//number of cells
	eNnrn = (mwSize)*mxGetPr(prhs[64]);
	iNnrn = (mwSize)*mxGetPr(prhs[65]);
	...
	plhs[23] = mxCreateDoubleMatrix(simulLen, 1, mxREAL);
	e2eI322 = mxGetPr(plhs[23]);

	plhs[24] = mxCreateDoubleMatrix(simulLen, 1, mxREAL);
	e2iI322 = mxGetPr(plhs[24]);

	plhs[25] = mxCreateDoubleMatrix(simulLen, 1, mxREAL);
	e2eI123 = mxGetPr(plhs[25]);

	plhs[26] = mxCreateDoubleMatrix(simulLen, 1, mxREAL);
	e2iI123 = mxGetPr(plhs[26]);
	...
	iX3 = mxCalloc(totNnrn, sizeof(double));
	uX3 = mxCalloc(totNnrn, sizeof(double));
	...
	iI3 = mxCalloc(totNnrn, sizeof(double));
	uI3 = mxCalloc(totNnrn, sizeof(double));
	...
	Ncon322 = mxCalloc(eNnrn, sizeof(mwSize));
	Neicon322 = mxCalloc(eNnrn, sizeof(mwSize));
	Ncon123 = mxCalloc(eNnrn, sizeof(mwSize));
	Neicon123 = mxCalloc(eNnrn, sizeof(mwSize));
	...
	eSPC322 = mxCalloc(eNnrn * eCycSizeinter, sizeof(double));
	eiSPC322 = mxCalloc(iNnrn * eCycSizeinter, sizeof(double));
	eSPC123 = mxCalloc(eNnrn * eCycSizeinter, sizeof(double));
	eiSPC123 = mxCalloc(iNnrn * eCycSizeinter, sizeof(double));
	...
	A322 = mxCalloc(eNnrn * eNnrn, sizeof(mwSize));
	Aei322 = mxCalloc(eNnrn * iNnrn, sizeof(mwSize));

	A123 = mxCalloc(eNnrn * eNnrn, sizeof(mwSize));
	Aei123 = mxCalloc(eNnrn * iNnrn, sizeof(mwSize));
	...
	for (nrn1 = 0; nrn1 < eNnrn; nrn1++) {
		...
		for (nrn2 = 0; nrn2 < eNnrn; nrn2++) {
			...
			if (rndNum < p) {
				A322[Ncon322[nrn1] + nrn1*eNnrn] = nrn2;
				Ncon322[nrn1]++;
			}
		}
	}
	...
	for (nrn1 = 0; nrn1 < eNnrn; nrn1++) {
		...
		for (nrn2 = 0; nrn2 < iNnrn; nrn2++) {
			...
			if (rndNum < p) {
				Aei322[Neicon322[nrn1] + nrn1*iNnrn] = nrn2;
				Neicon322[nrn1]++;
			}
		}
	}
	...
	for (nrn1 = 0; nrn1 < eNnrn; nrn1++) {
		...
		for (nrn2 = 0; nrn2 < eNnrn; nrn2++) {
			...
			if (rndNum < p) {
				A123[Ncon123[nrn1] + nrn1*eNnrn] = nrn2;
				Ncon123[nrn1]++;
			}
		}
	}
	...
	for (nrn1 = 0; nrn1 < eNnrn; nrn1++) {
		...
		for (nrn2 = 0; nrn2 < iNnrn; nrn2++) {
			...
			if (rndNum < p) {
				Aei123[Neicon123[nrn1] + nrn1*iNnrn] = nrn2;
				Neicon123[nrn1]++;
			}
		}
	}
	...
	for (t = 0; t < simulLen; t++) {
		...
		for (nrn = 0; nrn < totNnrn; nrn++) {
			...
			synI = xI0 + aI0 - gI0 + iI0 + uI0 + iI30 + uI30;
			...
			if (nrn < eNnrn){ //excitatory neurons area 1
				...
				if (V[nrn] > Vthr) { //spike generation
					...
					for (con = 0; con < Ncon123[nrn]; con++) {
						a = A123[con + nrn*eNnrn];/*you have conexions from nrn to others*/
						eSPC123[a + eCycSPinter]++;
					}
					for (con = 0; con < Neicon123[nrn]; con++) {
						a = Aei123[con + nrn*iNnrn];/*you have conexions from nrn to others*/
						eiSPC123[a + iCycSPinter]++;
					}
				}
				...
				e2eSPC322 = eSPC322[nrn - totNnrnt + eCycIndxinter];

				C = eTm2 * (e2eJ322 * e2eSPC322);

				iX30 = iX3[nrn];

				iX3_k1 = ((C - iX30) / e2eTr2) / 2;
				iI3_k1 = ((iX30 - iI30) / e2eTd2) / 2;


				iX3_k2 = ((C - iX30 - iX3_k1) / e2eTr2);
				iI3_k2 = ((iX30 - iI30 + iX3_k1 - iI3_k1) / e2eTd2);
				...
				synI_k1 = xI_k1 + aI_k1 - gI_k1 + iI_k1 + iI3_k1;
				...
				eSPC322[nrn - totNnrnt + eCycIndxinter] = 0;
			}
				...
				e2iSPC322 = eiSPC322[nrn - totNnrnt - eNnrn + iCycIndxinter];

				C = iTm2 * (e2iJ322 * e2iSPC322);

				uX30 = uX3[nrn];

				uX3_k1 = ((C - uX30) / e2iTr2) / 2;
				uI3_k1 = ((uX30 - uI30) / e2iTd2) / 2;


				uX3_k2 = ((C - uX30 - uX3_k1) / e2iTr2);
				uI3_k2 = ((uX30 - uI30 + uX3_k1 - uI3_k1) / e2iTd2);
				...
				synI_k1 = xI_k1 + aI_k1 - gI_k1 + uI_k1 + uI3_k1;
				...
				eiSPC322[nrn - totNnrnt - eNnrn + iCycIndxinter] = 0;
			}
				...
				e2eSPC123 = eSPC123[nrn - (totNnrnt * 2) + eCycIndxinter];

				C = eTm3 * (e2eJ123 * e2eSPC123);

				iX0 = iX[nrn];

				iX_k1 = ((C - iX0) / e2eTr3) / 2;
				iI_k1 = ((iX0 - iI0) / e2eTd3) / 2;


				iX_k2 = ((C - iX0 - iX_k1) / e2eTr3);
				iI_k2 = ((iX0 - iI0 + iX_k1 - iI_k1) / e2eTd3);
				...
				if (V[nrn] > Vthr) {
					...
					for (con = 0; con < Ncon322[nrn - (totNnrnt * 2)]; con++) {
						a = A322[con + (nrn - (totNnrnt * 2))*eNnrn];/*you have conexions from nrn to others*/
						eSPC322[a + eCycSPinter]++;
					}

					for (con = 0; con < Neicon322[nrn - (totNnrnt * 2)]; con++) {
						a = Aei322[con + (nrn - (totNnrnt * 2))*iNnrn];/*you have conexions from nrn to others*/
						eiSPC322[a + iCycSPinter]++;
					}
				}
				eSPC3[nrn - (totNnrnt * 2) + eCycIndx3] = 0;
				iSPC3[nrn - (totNnrnt * 2) + iCycIndx3] = 0;
				eSPC123[nrn - (totNnrnt * 2) + eCycIndxinter] = 0;
				...
				e2iSPC123 = eiSPC123[nrn - (totNnrnt * 2) - eNnrn + iCycIndxinter];

				C = iTm3 * (e2iJ123 * e2iSPC123);

				uX0 = uX[nrn];

				uX_k1 = ((C - uX0) / e2iTr3) / 2;
				uI_k1 = ((uX0 - uI0) / e2iTd3) / 2;


				uX_k2 = ((C - uX0 - uX_k1) / e2iTr3);
				uI_k2 = ((uX0 - uI0 + uX_k1 - uI_k1) / e2iTd3);
				...
				eSPC3[nrn - (totNnrnt * 2) + eCycIndx3] = 0;
				iSPC3[nrn - (totNnrnt * 2) + iCycIndx3] = 0;
				eiSPC123[nrn - (totNnrnt * 2) - eNnrn + iCycIndxinter] = 0;
			}
