...

{
	...
	e2eJ322 = *mxGetPr(prhs[58]);
	e2iJ322 = *mxGetPr(prhs[59]);

	e2eJ123 = *mxGetPr(prhs[60]);
	e2iJ123 = *mxGetPr(prhs[61]);

	//number of cells
	eNnrn = (mwSize)*mxGetPr(prhs[62]);
	iNnrn = (mwSize)*mxGetPr(prhs[63]);
	...
	for (t = 0; t < simulLen; t++) {
		...
		for (nrn = 0; nrn < totNnrn; nrn++) {
			...
			if (nrn < eNnrn){ //excitatory neurons area 1
				...
				synI_k1 = xI_k1 + aI_k1 - gI_k1 + iI3_k1;
				...
				synI_k1 = xI_k1 + aI_k1 - gI_k1 + uI3_k1;
